...
Class Master extends DBConnection {
	...
	function save_product(){
		...
		if($save){
			...
			if(!isset($resp)){
				$resp['status'] = 'success';
				if(empty($id))
					$this->settings->set_flashdata('success',"Product successfully saved.");
				else
    ...
    function save_blog(){
		...
		$check = $this->conn->query("SELECT * FROM `blog_posts` where `title` = '{$title}' ".(!empty($id) ? " and id != {$id} " : "")." ")->num_rows;
		if($this->capture_err())
			return $this->capture_err();
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "blog already exist.";
			return json_encode($resp);
			exit;
		}
		if(empty($id)){
			$sql = "INSERT INTO `blog_posts` set {$data} ";
		}else{
			$sql = "UPDATE `blog_posts` set {$data} where id = '{$id}' ";
		}
		$save = $this->conn->query($sql);
		if($save){
			$pid = empty($id) ? $this->conn->insert_id : $id;
			$upload_path = "uploads/blog_".$pid;
			if(!is_dir(base_app.$upload_path))
				mkdir(base_app.$upload_path);
			if(isset($_FILES['img']) && count($_FILES['img']['tmp_name']) > 0){
				$err = "";
				...
				if(!empty($err)){
					$resp['status'] = 'failed';
					$resp['msg'] = 'blog successfully saved but '.$err;
					$resp['id'] = $pid;
				}
			}
			if(!isset($resp)){
				$resp['status'] = 'success';
				if(empty($id))
					$this->settings->set_flashdata('success',"blog successfully saved.");
				else
	...
	function delete_blog(){
		extract($_POST);
		$del = $this->conn->query("DELETE FROM `blog_posts` where id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"blog_posts successfully deleted.");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
